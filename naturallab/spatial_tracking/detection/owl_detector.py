...
class OWLDetectorModule(TrackerModule):
    ...
    def __init__(self, 
        ...
        try:
            ...
            self.load_error = (
                "transformers is unavailable; install the spatial extra"
            )
            self.log_error(f"Failed to load OWLv2: {self.load_error}")
            self.has_model = False
        except Exception as e:
            self.load_error = f"{type(e).__name__}: {e}"
            self.log_error(f"Failed to load OWLv2 model: {self.load_error}")
            self.has_model = False
    ...
    def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.has_model:
            raise RuntimeError(
                "OWLv2 inference is unavailable because model initialization "
                f"failed: {self.load_error}"
            )
