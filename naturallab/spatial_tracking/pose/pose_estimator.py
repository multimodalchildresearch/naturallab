    ...
    def _filter_outlier_landmarks(self, landmarks, bbox, frame_shape):
        ...
        for i, lm in enumerate(landmarks.landmark):
            ...
            if should_filter:
                ...
                reason = []
                if x_min_violation:
                    reason.append(f"x < min by {x1 - lm.x:.1f}")
                if x_max_violation:
                    reason.append(f"x > max by {lm.x - x2:.1f}")
                if y_min_violation:
                    reason.append(f"y < min by {y1 - lm.y:.1f}")
                if y_max_violation:
                    reason.append(f"y > max by {lm.y - y2:.1f}")
                if metadata['outside_frame']:
                    reason.append("outside frame")
