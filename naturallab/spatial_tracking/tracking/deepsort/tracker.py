...
from naturallab.spatial_tracking.diagnostics.deepsort_diagnostics import DeepSORTDiagnostics
...
from naturallab.spatial_tracking.tracking.deepsort.feature_gallery import (
    PersonFeatureGallery,
)
from naturallab.spatial_tracking.utils.data_structures import generate_track_id


class DeepSORTUnavailableError(RuntimeError):
    """Raised when the requested DeepSORT quality backend is unavailable."""
...
class DeepSORTTracker(BaseTracker):
    ...
    def __init__(self, max_age: int = 30, min_hits: int = 3, iou_threshold: float = 0.3,
                 ...
                 reid_model_architecture: str = 'osnet_ain_x1_0',
        ...
        Args:
            ...
            reid_model_architecture: Explicit checkpoint architecture
            ...
            require_reid_model: Deprecated strict-mode alias. Passing false
                now raises and points to ``allow_reid_fallback``
            feature_extractor: Optional injected appearance feature extractor
            feature_gallery: Optional injected feature gallery
        """
        ...
        if require_reid_model is not None:
            ...
            warnings.warn(
                "require_reid_model is deprecated; use "
                "allow_reid_fallback instead",
                DeprecationWarning,
                stacklevel=2,
            )
            if allow_reid_fallback and require_reid_model:
                raise ValueError(
                    "require_reid_model=True conflicts with "
                    "allow_reid_fallback=True"
                )
            if not require_reid_model:
                raise ValueError(
                    "require_reid_model=False no longer enables fallback; "
                    "set allow_reid_fallback=True explicitly"
                )
        
        # Add this line to track all used IDs
        self.used_track_ids = set()
        # Model injection keeps construction testable without downloading a
        # checkpoint. The quality preset requires a real ReID model and fails
        # clearly instead of silently degrading to histogram-only features.
        self.feature_extractor = (
            feature_extractor
            if feature_extractor is not None
            else AppearanceFeatureExtractor(
                model_path=reid_model_path,
                model_architecture=reid_model_architecture,
                device=reid_device,
            )
        )
        model_ready = bool(getattr(
            self.feature_extractor,
            "has_model",
            False,
        )) and callable(
            getattr(self.feature_extractor, "extract_deep_features", None)
        )
        self.reid_failure_category = None
        if not model_ready:
            model_error = getattr(
                self.feature_extractor,
                "model_error",
            ...
            model_failure = (
                "the configured DeepSORT ReID model could not be loaded; "
                "verify the checkpoint, PyTorch installation, and reid_device"
            )
            message = (
                model_failure
                + ". allow_reid_fallback=True was explicitly set; using "
                "48-D HSV histogram features for the entire run"
                if allow_reid_fallback
                else fallback_guidance(model_failure)
            )
            warnings.warn(message, ReIDCheckpointWarning, stacklevel=2)
            if not allow_reid_fallback:
                error = DeepSORTUnavailableError(message)
                if isinstance(model_error, BaseException):
                    raise error from model_error
                raise error
            if not callable(
                getattr(
                    self.feature_extractor,
                ...
                raise DeepSORTUnavailableError(
                    "allow_reid_fallback=True requires an extractor with "
                    "extract_fallback_features()"
                )
            self.reid_backend = "histogram"
            self.reid_failure_category = "model-load-failed"
        else:
            self.reid_backend = reid_model_architecture
        
        # Initialize the feature gallery
        self.feature_gallery = (
            feature_gallery
            if feature_gallery is not None
            else PersonFeatureGallery(
                max_features_per_id=30,  # Store more features per ID
                max_inactive_time=keep_lost_timeout,
        ...
        self.allow_reid_fallback = allow_reid_fallback
        self.require_reid_model = not allow_reid_fallback
    ...
    def _create_new_track(self, detection: np.ndarray) -> Dict[str, Any]:
        ...
        while new_track['id'] in self.used_track_ids:
            ...
            new_track['id'] = generate_track_id()
    ...
    def _extract_appearance_features(
        ...
        """Extract ReID features without quality-path runtime fallback."""

        if self.reid_backend == "histogram":
            return self.feature_extractor.extract_fallback_features(crop)
        deep_extractor = getattr(
            self.feature_extractor,
            "extract_deep_features",
            None,
        )
        if not callable(deep_extractor):
            raise DeepSORTUnavailableError(
                "strict DeepSORT requires an injected feature extractor with "
                "extract_deep_features(); runtime fallback is disabled"
            )
        try:
            features = deep_extractor(crop)
        except Exception as error:
            raise DeepSORTUnavailableError(
                "DeepSORT ReID inference failed at runtime. The tracker will "
                "not switch feature dimensions during an active run"
            ) from error
        if features is None:
            model_error = getattr(
                self.feature_extractor,
            ...
            error = DeepSORTUnavailableError(
                "DeepSORT ReID inference returned no deep features. The "
                "tracker will not switch feature dimensions during an active "
                "run"
            )
            if isinstance(model_error, BaseException):
                raise error from model_error
    ...
    def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detection_metadata = data.get('detection_metadata') or {}
        intentional_skip = detection_metadata.get('skipped') is True
        ...
        for track in self.tracks:
            ...
            track['bbox'] = self._bbox_from_kalman(track['kalman'])
            track['is_prediction'] = True
        ...
        # A Qwen cadence skip means no observation was attempted. Return motion
        # predictions without aging tracks, expiring gallery entries, or
        # interpreting the empty detections list as evidence of disappearance.
        if intentional_skip:
            return {
        ...
        if unmatched_detections and self.lost_tracks:
            ...
            for det_idx in unmatched_detections:
                ...
                det_features = self._extract_appearance_features(det_crop)
                ...
                if best_lost_idx >= 0 and best_score > adaptive_threshold:
                    ...
                    lost_track['hits'] += 1
                    lost_track['is_prediction'] = False
                    ...
                    if crop is not None:
                        features = self._extract_appearance_features(crop)
                        if self.diagnostics is not None:
        ...
        for track_idx, det_idx in matches:
            ...
            self.tracks[track_idx]['is_prediction'] = False
    ...
    def _active_tracks(self) -> List[Dict[str, Any]]:
        """Select publishable tracks consistently on observed and skip frames."""
        ...
        for track in self.tracks:
            ...
            if (
                ...
                and track['time_since_update'] <= time_threshold
            ):
