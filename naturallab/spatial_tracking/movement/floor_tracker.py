...
class EnhancedFloorTracker:
    ...
    def __init__(self, camera_matrix, dist_coeffs, floor_plane, correction_factor=1.0,
        ...
        self.logger = logging.getLogger(
            "naturallab.spatial_tracking.EnhancedFloorTracker"
        )
...
class SimpleFloorTracker:
    ...
    def __init__(
        ...
        Args:
            ...
            floor_plane: Floor plane equation
            correction_factor: Factor to apply to distances (e.g., 0.83 to reduce by 17%)
            units: Units used by the floor plane and returned coordinates.
        """
        self.logger = logging.getLogger(
        ...
        millimetres_per_unit = unit_to_mm.get(str(units).strip().lower())
        if millimetres_per_unit is None:
            self.min_movement = 0.0
            self.max_movement = float("inf")
            self.logger.warning(
                "Unknown calibration units %r; movement outlier thresholds "
                "are disabled",
                units,
            )
        else:
            self.min_movement = 5.0 / millimetres_per_unit
            self.max_movement = 200.0 / millimetres_per_unit
    ...
    def update_track(self, track_id, bbox):
        ...
        if len(self.positions[track_id]) >= 2:
            ...
            # Filter out unrealistic movements
            if self.min_movement < raw_distance < self.max_movement:
                # Add to raw total
