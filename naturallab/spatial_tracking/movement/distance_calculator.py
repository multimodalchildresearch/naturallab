...
class DistanceCalculator:
    ...
    def __init__(self, correction_factor: float = 1.0):
        ...
        self.logger = logging.getLogger(
            "naturallab.spatial_tracking.DistanceCalculator"
        )
        ...
        self.accepted_steps = {}   # track_id -> accepted raw step distances
    ...
    def add_position(self, track_id: str, position: np.ndarray, 
        ...
        if min_movement <= raw_distance <= max_movement:
            ...
            self.accepted_steps[track_id].append(raw_distance)
    ...
    def get_distance_metrics(self, track_id: str) -> Dict[str, float]:
        ...
        if len(positions) >= 2:
            steps = self.accepted_steps.get(track_id, [])
            if steps:
    ...
    def apply_custom_correction(self, correction_function: Callable[[float], float]) -> None:
        ...
        self.logger.info("Applied custom correction function to all distances")
    ...
    def reset(self, track_id: Optional[str] = None) -> None:
        ...
        if track_id is not None:
            ...
            if track_id in self.accepted_steps:
                del self.accepted_steps[track_id]
