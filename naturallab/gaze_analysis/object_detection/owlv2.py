...
class OWLv2Detector:
    """Compatibility wrapper exposing the script-level ``detect`` contract.

    New pipeline code should depend on a detector protocol rather than this
    concrete class. This wrapper keeps the existing standalone script usable
    while that migration proceeds.
    """
    ...
    def detect(
        ...
        *,
        ...
        if not queries:
            raise ValueError("queries must contain at least one label")
        if not isinstance(image, Image.Image):
            raise TypeError("image must be a PIL.Image.Image")

        pixel_values = self.backend.process_image(image)
        text_inputs = self.backend.process_text(queries)
        raw = self.backend.text_guided_detection(
            **text_inputs,
            pixel_values=pixel_values,
        )
        output = post_process_batch(
            raw,
        ...
        boxes = box_convert(
            scale_bounding_boxes(
                ...
                model_size=self.backend.config.vision_config.image_size,
            ),
            in_fmt="cxcywh",
            out_fmt="xywh",
        )
        ...
        return {
            ...
            "labels": [queries[index] for index in output.labels.tolist()],
        }
