...
- Optional source cleanup after successful extraction (disabled by default)
...
try:
    ...
    # Acquisition helpers remain usable on headless compute nodes where Tk's
    # optional native libraries (for example libX11) are not installed.
    tk = None
...
_SENSITIVE_CONFIG_KEYS = frozenset({"rtsp_pass", "rtsp_password"})
_UNCONFIGURED_ADDRESSES = frozenset({"", "YOUR_IP_ADDRESS"})
_TK_NORMAL = "normal"
...
def _require_tk():
    """Fail clearly when the optional desktop GUI dependencies are unavailable."""
    if tk is None:
        message = (
            "The recording GUI requires Python Tk support and its native display "
            "libraries. Install them and run this command in a graphical session."
        )
        raise RuntimeError(message) from _TK_IMPORT_ERROR


def _labrecorder_quoted(value):
    """Quote one LabRecorder list value using its QSettings syntax."""
    text = str(value)
    if "\n" in text or "\r" in text:
        raise ValueError("LabRecorder values cannot contain newlines")
    return '"' + text.replace("\\", "\\\\").replace('"', '\\"') + '"'


def build_labrecorder_config(recording_dir, required_streams, hostname):
    """Build a current LabRecorder key/value configuration file."""
    recording_root = Path(recording_dir).expanduser()
    storage_location = recording_root / "exp%n" / "block_%b.xdf"
    if any(character in str(storage_location) for character in "\r\n"):
        raise ValueError("The recording directory cannot contain newlines")
    host = str(hostname).strip()
    if not host or any(character in host for character in "\r\n()"):
        raise ValueError("A valid LSL source hostname is required")

    stream_entries = [
        _labrecorder_quoted(f"{name} ({host})")
        for name in dict.fromkeys(str(item).strip() for item in required_streams)
        if name
    ]
    lines = [
        "; NaturalLab LabRecorder configuration",
        "; Generated in the upstream key/value .cfg format.",
        f"StorageLocation={storage_location}",
        'SessionBlocks="Acceptance","Main"',
    ]
    if stream_entries:
        lines.append("RequiredStreams=" + ",".join(stream_entries))
    lines.extend(
        [
            "RCSEnabled=1",
            "RCSPort=22345",
...
def build_rtsp_url(host, stream, username="", password=""):
    """Build an RTSP URL while safely encoding its user information."""
    host = host.strip()
    if not host:
        raise ValueError("An RTSP camera host is required")
    if "://" in host or "@" in host or "/" in host:
        raise ValueError("Enter only the RTSP camera host or host:port")

    stream = stream.strip().lstrip("/") or "stream1"
    ...
    return f"rtsp://{userinfo}{host}/{stream}"


def redact_rtsp_url(url):
    """Return an RTSP URL with any password replaced by a fixed marker."""
    scheme_end = url.find("://")
    ...
    authority = url[authority_start:path_start]
    if "@" not in authority:
        return url
    ...
    if ":" in userinfo:
        ...
        userinfo = f"{username}:***"
    return f"{url[:authority_start]}{userinfo}@{host}{url[path_start:]}"


def redact_command(command):
    """Redact credentials from RTSP URL arguments before logging a command."""
    redacted = list(command)
    for index, argument in enumerate(redacted[:-1]):
        if argument == "--rtsp-urls":
            redacted[index + 1] = ",".join(
                redact_rtsp_url(url) for url in redacted[index + 1].split(",")
            )
    return redacted


def redact_log_text(value):
    """Remove RTSP passwords from free-form subprocess output before logging."""
    return _RTSP_URL_PATTERN.sub(
...
def _sanitized_config(config):
    """Copy a config while removing legacy persisted RTSP password fields."""
    if not isinstance(config, dict):
        raise ValueError("Recording configuration must be a JSON object")
...
class RecordingGUI:
    def __init__(self, root):
        ...
        self._config_migration_notice = ""
        self._config_security_warning = ""
        ...
        if self._config_migration_notice:
            self.log(self._config_migration_notice)
        if self._config_security_warning:
            self.log(f"WARNING: {self._config_security_warning}")
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r', encoding="utf-8") as f:
                    config = json.load(f)
                config, removed_keys = _sanitized_config(config)
                ...
                if removed_keys:
                    try:
                        ...
                        self._config_migration_notice = (
                            "Removed a legacy plaintext RTSP password from the saved "
                            "configuration. Enter it again for this session."
                        )
                    except OSError as error:
                        self._config_security_warning = (
                            "A legacy RTSP password was not loaded, but it could not be "
                            f"removed from {self.config_file}: {error}"
                        )
                return config
    ...
    def save_config(self):
        ...
        try:
            self.config, _removed_keys = _sanitized_config(self.config)
            _write_private_json(self.config_file, self.config)
        except Exception as e:
    ...
    def setup_main_tab(self, notebook):
        ...
        self.stop_lsl_btn = ttk.Button(lsl_frame, text="Stop LSL Streaming", 
                                      command=self.stop_lsl_streaming, state=_TK_DISABLED)
        ...
        self.close_labrecorder_btn = ttk.Button(labrecorder_frame, text="Close LabRecorder", 
                                               command=self.close_labrecorder, state=_TK_DISABLED)
        ...
        self.rtsp_pass_var = tk.StringVar(value="")
        ttk.Entry(cred_frame, textvariable=self.rtsp_pass_var, width=10, show="*").pack(side=tk.LEFT, padx=2)
        ttk.Label(cred_frame, text="(session only)", foreground="gray").pack(side=tk.LEFT, padx=(2, 8))
        ...
        # Stream preview matching the configured source script
...
Note: Eye events (fixations/saccades) only work for Child device due to API limitations."""
    ...
    def create_labrecorder_config(self):
        """Create a default LabRecorder config file for a multi-device setup."""
        ...
        for role in configured_roles:
            required_streams.extend(
                [f"NeonVideo_{role}", f"NeonGaze_{role}"]
            )
            if not self.no_imu_var.get():
                required_streams.append(f"NeonIMU_{role}")
            if not self.no_audio_var.get():
                required_streams.append(f"NeonAudio_{role}")
        if "Child" in configured_roles and not self.no_eye_events_var.get():
            required_streams.extend(
                ["NeonFixations_Child", "NeonSaccades_Child"]
            )
        if not self.no_realsense_var.get():
            required_streams.extend(
                ["RealSense_Color", "RealSense_Depth", "RealSense_Metadata"]
            )
        for camera in self.camera_vars.values():
            if (
                camera["enabled"].get()
                and camera["ip"].get().strip() not in _UNCONFIGURED_ADDRESSES
            ):
                required_streams.append(camera["name"].get().strip())

        try:
            config_content = build_labrecorder_config(
                recordings_dir,
                required_streams,
                socket.gethostname(),
            )
            ...
            messagebox.showinfo("Config Created", 
                               f"Multi-device LabRecorder config created successfully!\n\nPath: {config_path}\n\nRequired-stream warnings were generated from the devices currently configured in the GUI. LabRecorder will write exp%n/block_%b.xdf below the selected recording directory. Review the stream list after clicking Update.\n\nThe config is now set as your LabRecorder config file.")
            ...
            if cam_key in self.camera_vars:
                ...
                if ip and name:
                    try:
                        ...
                        urls_info += f"Camera {i}: ⚠️ INVALID ({error})\n\n"
                        continue
                    status = "✅ ENABLED" if enabled else "❌ DISABLED"
                    urls_info += f"Camera {i}: {name}\n"
                    urls_info += f"  URL: {redact_rtsp_url(rtsp_url)}\n"
    ...
    def save_current_config(self):
        ...
        self.config.pop("rtsp_pass", None)
        self.config.pop("rtsp_password", None)
        ...
        self.save_config()
        self.log("Configuration saved (RTSP password was not stored)")
        messagebox.showinfo(
            "Saved",
            "Configuration saved successfully.\n\n"
            "The RTSP password is session-only and was not stored.",
        )
    ...
    def start_lsl_streaming(self):
        ...
        try:
            ...
            self.stop_lsl_btn.config(state=_TK_NORMAL)
            ...
            safe_error = redact_log_text(e)
            self.log(f"Error starting LSL streaming: {safe_error}")
            messagebox.showerror(
                "Error",
                f"Failed to start LSL streaming: {safe_error}",
            )
    ...
    def stop_lsl_streaming(self):
        ...
        process_pid = getattr(self.lsl_process, "pid", None)
        self.log(f"lsl_process_pid: {process_pid if process_pid else 'none'}")
    ...
    def start_labrecorder(self):
        ...
        try:
            ...
            self.start_labrecorder_btn.config(state=_TK_DISABLED)
            self.close_labrecorder_btn.config(state=_TK_NORMAL)
    ...
    def close_labrecorder(self):
        ...
        self.start_labrecorder_btn.config(state=_TK_NORMAL)
        self.close_labrecorder_btn.config(state=_TK_DISABLED)
    ...
    def _extract_file_thread(self, filepath):
        ...
        try:
            ...
            error_message = str(e)
            self.log(f"Error during extraction: {error_message}")
            self.root.after(
                0,
                lambda message=error_message: messagebox.showerror(
                    "Error",
                    f"Error during extraction: {message}",
                ),
            )
